...
1. The User Flow
Create/Join: A user creates a group on the web app, paying an entry fee in SOL via their Phantom wallet. They receive an invite code.
Link Telegram: The group creator sends the invite link to their Telegram bot (/start LINK_<invite_code>), binding the web group to their Telegram chat.
Predict: Members join the web app, connect their wallet, pay the entry fee, and make their 4 picks before kickoff.
Live Match: Once the match starts, predictions lock. The backend constantly polls TxLINE for events.
Goal!: When a goal happens, the game engine partially evaluates picks (e.g., First Goalscorer). The bot crafts a TTS script detailing who scored and whose predictions were ruined, generating a voice note and sending it to Telegram.
Full-Time: The backend evaluates all final predictions, updates the leaderboard, and executes the Solana payout to the highest scorer.
